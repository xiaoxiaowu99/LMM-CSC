...
class  OpenaiAPI:
    def __init__(self, api_key_list, model_name:str="gpt-3.5-turbo", temperature:float=0.05, max_tokens: int=4096):
    ...
    def send_request_turbo(self, prompt, question, zero_shot):
        """
        """
        if zero_shot == False:
    ...
    def forward(self, prompt, question, zero_shot)->str:
        """
        """
        if self.model_name == "gpt-3.5-turbo":
            output = self.send_request_turbo(prompt, question, zero_shot)
    ...
    def __call__(self, prompt:str, question:str):
        if prompt == '':
            ...
            zero_shot = False
        return self.forward(prompt, question, zero_shot)
...
if __name__ == "__main__":
    api_key_list = ["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"]
